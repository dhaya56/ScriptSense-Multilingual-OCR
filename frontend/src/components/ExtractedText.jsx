...
const ExtractedText = ({
  ...
  // Sync displayedText and editableText when the 'text' prop changes (e.g., new OCR result)
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newText = result.corrected_text || editableText;
      ...
      await saveEditedText({ text: newText, area: 'extracted', email });
    } catch (err) {
  ...
  const downloadPdfFromFrontend = () => {
    ...
    doc.save(`extracted_text_${lang}.pdf`); // You might want to include lang code in filename
    setShowDownloadOptions(false);
  };

  const downloadDocxFromFrontend = () => {
    const content = displayedText;
    const filename = 'extracted_text.doc'; // Using .doc for basic compatibility with Word
  ...
  return (
    <div className="border border-gray-200 bg-white rounded-xl p-5 shadow-sm mt-6 relative">
      {/* Top Header */}
      <div className="flex justify-between items-start mb-2">
        <h2 className="text-sm font-semibold text-purple-700">📝 Extracted Text</h2>
        {!editing && (
          <button
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-md p-4 min-h-[10rem] mb-4">
        {editing ? (
          ...
          <p className="whitespace-pre-wrap text-sm text-gray-800">{displayedText}</p>
        )}
      </div>

      {/* Controls */}
      <div className="flex flex-wrap items-center gap-3 justify-start">
        <TextToSpeechButton text={displayedText} lang={lang} />
